...
const EventDetails = ({ params: { id }, searchParams }: SearchParamProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const authUser = await currentUser();
        const fetchedUser = await getUserByClerkId(
          authUser?.id || "user_2i4XYyrgQ1Xo9IvjuydCOGCLmZ6"
        );
        ...
        setAttending(
          fetchedEvent.attendees.some((a: string) => a === fetchedUser._id)
        );
        setSoldOut(
          fetchedEvent.noLimit
            ? false
            : (fetchedEvent.attendees.length ?? 0) >= fetchedEvent.limit
        );
        setLoading(false);
      } catch (error) {
        ...
        <div className="grid grid-cols-1  md:grid-cols-2 2xl:max-w-7xl">
          ...
          <div className="flex w-full flex-col gap-8 p-5 md:p-10">
            ...
            {user._id !== event.organizer._id &&
              ...
              !soldOut &&
              !event.canceled && (
                <CheckoutButton event={event} userId={user._id} />
              )}
